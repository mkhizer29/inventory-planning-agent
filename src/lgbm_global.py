"""lgbm_global.py — Aqib. ONE gradient-boosted (LightGBM) model across all 30 pilot SKUs.

Contract (v4, real-demand / synthetic-stock, daily / naheed_web-only — see
Doc3_Aqib_LightGBM.md's "READ THIS FIRST" callout and pilot_manifest.json):
  * Inputs: data/processed/model_panel.parquet (train/backtest) and
    data/processed/forecast_frame.parquet (future, no actuals). Read-only — never
    regenerated or edited here (that's prepare_pilot_data.py, the pipeline owner's job).
  * Target: REAL units_observed. Row eligibility for both training and scoring is
    forecast_training_eligible (real-data quality + >=14 days history) — never based
    on synthetic stock. evaluate() asserts this independence at runtime.
  * Features: the manifest's demand_feature_whitelist, read at runtime (not hardcoded)
    so this script tracks pilot_manifest.json if it ever changes:
      units_lag_1/7/14, units_roll_mean_7/28, units_roll_std_7, effective_unit_price,
      discount_pct, on_promo, is_public_holiday, is_payday_window, day_of_week,
      is_weekend, week_of_year, month, is_ramadan, ramadan_day, ramadan_week
    Plus ONE addition beyond the whitelist: `sku` as a categorical grouping feature.
    This is a POOLED model (one model, all 30 SKUs stacked) — unlike Khizer's
    per-SKU Holt-Winters, it needs a SKU identity feature to tell products apart and
    to let the tree structure borrow shared patterns across SKUs. `channel` is not
    a feature (constant "naheed_web" for the whole pilot, no variation to learn from).
    NEVER fed to the model (checked at runtime): stock_on_hand, unit_cost,
    unit_cost_observed, unit_cost_effective, or any other model_panel column outside
    the whitelist + sku.
  * Split: evaluation.py's locked chronological backtest_split(panel, horizon) for
    horizon in (7, 14) — identical cutoff/train/test as Aiman's baselines and
    Khizer's Holt-Winters. Never a random split.
  * Multi-day horizon: lag/rolling features for backtest test-days come straight from
    model_panel (they're real, causally shifted at construction — see
    prepare_pilot_data.py's `grp.shift(1)` before rolling — so using them for held-out
    real days is not leakage: date t's lag/rolling only reads real values strictly
    before t). The FUTURE forecast (forecast_frame.parquet) has no such precomputed
    lag/rolling columns since no real future demand exists yet, so those 14 days are
    forecast recursively per SKU: predict day 1, fold that prediction into the
    SKU's running series, recompute lag/rolling for day 2 from that series, etc.
  * Scoring: evaluate(preds, horizon=h, panel=panel) from src/evaluation.py — imported,
    never edited. preds carries only [sku, channel, date, y_pred] (never y_true).
    Reports WAPE (primary), MAE, MASE, RMSE, bias per the shared contract.

Run:  python src/lgbm_global.py            (writes outputs/)
"""
from __future__ import annotations

import argparse
import json
import sys
import warnings
from pathlib import Path

import lightgbm as lgb
import numpy as np
import pandas as pd

REPO_ROOT = Path(__file__).resolve().parents[1]
SRC = REPO_ROOT / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

import evaluation as ev  # shared scorecard — imported, never edited  # noqa: E402
import model_contract as mc  # shared run-aware forecasting contract  # noqa: E402

PROC = REPO_ROOT / "data" / "processed"
OUT = REPO_ROOT / "outputs"
DEFAULT_MODEL_PANEL = PROC / "model_panel.parquet"
DEFAULT_FORECAST_FRAME = PROC / "forecast_frame.parquet"
MANIFEST_PATH = PROC / "pilot_manifest.json"

EXPECTED_SCHEMA = "4.0-real-demand-synthetic-stock"
EXPECTED_CHANNEL = "naheed_web"
TARGET = "units_observed"
HORIZONS = ev.HORIZONS  # (7, 14) — from the shared evaluator
MODEL_NAME = "lightgbm"          # shared-contract model identity (the `model` column)
MODEL_VERSION = "lgbm_global_v1"  # specific implementation version

# Target-derived features: in a fixed-origin recursive backtest these must be rebuilt from
# the running series (real history through the cutoff + the model's OWN earlier predictions),
# NEVER read from held-out rows (that would be teacher-forcing leakage).
TARGET_DERIVED_FEATURES = (
    "units_lag_1", "units_lag_7", "units_lag_14",
    "units_roll_mean_7", "units_roll_mean_28", "units_roll_std_7",
)

# Columns that must NEVER reach the model, whitelist or not (fail loudly if any leak in).
FORBIDDEN_INPUTS = (
    "stock_on_hand", "stock_on_hand_is_synthetic", "stock_source", "stock_generation_version",
    "unit_cost", "unit_cost_observed", "unit_cost_effective",
)

# Recursive-future feature name -> forecast_frame.parquet column name
# (only for the exogenous/calendar features; lag/rolling are simulated, see below)
FUTURE_COLUMN_MAP = {
    "effective_unit_price": "latest_known_price",
    "on_promo": "planned_promo",
    "discount_pct": "planned_discount_pct",
    # identical names in both frames:
    "is_public_holiday": "is_public_holiday",
    "is_payday_window": "is_payday_window",
    "day_of_week": "day_of_week",
    "is_weekend": "is_weekend",
    "week_of_year": "week_of_year",
    "month": "month",
    "is_ramadan": "is_ramadan",
    "ramadan_day": "ramadan_day",
    "ramadan_week": "ramadan_week",
}


# ── contract load + audit ───────────────────────────────────────────────────────────
def load_contract(model_panel=None, forecast_frame=None,
                  manifest=None) -> tuple[pd.DataFrame, pd.DataFrame, dict]:
    """Load the three shared inputs. None -> data/processed defaults; else the given paths."""
    panel = ev.load_model_panel(model_panel)
    future = ev.load_forecast_frame(forecast_frame)
    man_path = Path(manifest) if manifest else MANIFEST_PATH
    manifest = json.loads(Path(man_path).read_text(encoding="utf-8"))

    if manifest.get("schema_version") != EXPECTED_SCHEMA:
        raise RuntimeError(
            f"pilot_manifest.json schema_version={manifest.get('schema_version')!r}, "
            f"expected {EXPECTED_SCHEMA!r} — contract may have changed, re-check before training."
        )
    if manifest.get("channel_scope") != [EXPECTED_CHANNEL]:
        raise RuntimeError(f"unexpected channel_scope: {manifest.get('channel_scope')}")
    if set(panel["channel"].unique()) != {EXPECTED_CHANNEL} or set(future["channel"].unique()) != {EXPECTED_CHANNEL}:
        raise RuntimeError("model_panel/forecast_frame contain a channel other than naheed_web")
    if sorted(panel["sku"].unique()) != sorted(manifest["selected_skus"]):
        raise RuntimeError("model_panel SKU set does not match pilot_manifest.json selected_skus")
    if list(manifest["forecast_horizon_days"]) != list(HORIZONS):
        raise RuntimeError(
            f"manifest horizons {manifest['forecast_horizon_days']} != evaluation.HORIZONS {HORIZONS}"
        )
    as_of = pd.Timestamp(manifest["as_of_date"])
    if panel["date"].max() != as_of:
        raise RuntimeError(f"model_panel max date {panel['date'].max()} != manifest as_of_date {as_of}")

    return panel, future, manifest


def feature_list(manifest: dict) -> list[str]:
    """Manifest's demand_feature_whitelist + `sku` (pooled-model grouping feature, not in
    the whitelist itself — see module docstring for why it's added)."""
    whitelist = list(manifest["demand_feature_whitelist"])
    for col in FORBIDDEN_INPUTS:
        if col in whitelist:
            raise RuntimeError(f"forbidden column {col!r} found inside demand_feature_whitelist")
    return whitelist + ["sku"]


def _assert_no_forbidden_columns(cols: list[str]) -> None:
    leaked = set(cols) & set(FORBIDDEN_INPUTS)
    if leaked:
        raise RuntimeError(f"forbidden columns about to be fed to the model: {leaked}")


# ── LightGBM config (small-data discipline: 30 SKUs, ~6 months) ────────────────────
def _lgbm_params(**extra) -> dict:
    params = dict(
        objective="tweedie",
        tweedie_variance_power=1.3,
        n_estimators=1000,
        learning_rate=0.05,
        num_leaves=31,
        min_child_samples=20,
        subsample=0.8,
        colsample_bytree=0.8,
        random_state=42,
        verbosity=-1,
    )
    params.update(extra)
    return params


def _prep_X(df: pd.DataFrame, features: list[str], sku_categories: pd.CategoricalDtype) -> pd.DataFrame:
    _assert_no_forbidden_columns(features)
    X = df[features].copy()
    X["sku"] = X["sku"].astype(sku_categories)
    return X


# ── leakage-free recursive backtest ──────────────────────────────────────────────────
# Backtest exogenous-feature policy: at a fixed forecast origin (the cutoff), REALIZED
# held-out price/discount/promotion are NOT yet known, so they must not be read from
# held-out rows. We substitute only origin-known values; calendar fields stay held-out
# because they are genuinely known in advance. (The FUTURE path is unaffected — it uses
# forecast_frame's planned latest_known_price / planned_promo / planned_discount_pct.)
ORIGIN_SAFE_EXOG = ("effective_unit_price", "discount_pct", "on_promo")
BACKTEST_EXOG_POLICY = {
    "effective_unit_price": "most recent price known on/before the cutoff, carried forward",
    "on_promo": "0 (no promotion plan known at the forecast origin)",
    "discount_pct": "0 (no planned discount known at the forecast origin)",
    "calendar_and_known_in_advance": (
        "read from the held-out date: day_of_week, is_weekend, week_of_year, month, "
        "is_public_holiday, is_payday_window, is_ramadan, ramadan_day, ramadan_week"),
    "target_derived": "units_observed / units_lag_* / units_roll_* NEVER read from held-out rows",
}


def _recursive_feature_row(sku: str, date: pd.Timestamp, series: pd.Series,
                           exo_row: pd.Series, features: list[str], origin_exog: dict) -> dict:
    """One feature row for (sku, date) in a HELD-OUT window.

    * Target-derived lag/rolling features are rebuilt from `series` (real history through the
      cutoff + the model's own predictions for earlier held-out days), reading only points
      strictly before `date`.
    * Price/discount/promotion come from `origin_exog` — origin-known values (see
      BACKTEST_EXOG_POLICY), NOT the realized held-out values.
    * All other features are genuinely known-in-advance calendar fields read from the held-out
      row by name (day_of_week, is_weekend, week_of_year, month, holiday/payday/Ramadan flags).
    units_observed / units_lag_* / units_roll_* from the held-out row are NEVER read.
    """
    before = series[series.index < date]
    window7 = before.iloc[-7:] if len(before) else before
    window28 = before.iloc[-28:] if len(before) else before
    derived = {
        "units_lag_1": before.iloc[-1] if len(before) >= 1 else np.nan,
        "units_lag_7": series.get(date - pd.Timedelta(days=7), np.nan),
        "units_lag_14": series.get(date - pd.Timedelta(days=14), np.nan),
        "units_roll_mean_7": window7.mean() if len(window7) else np.nan,
        "units_roll_mean_28": window28.mean() if len(window28) else np.nan,
        "units_roll_std_7": window7.std() if len(window7) >= 2 else np.nan,
    }
    row = {}
    for f in features:
        if f == "sku":
            row[f] = sku
        elif f in derived:
            row[f] = derived[f]
        elif f in origin_exog:
            row[f] = origin_exog[f]  # origin-safe price/discount/promo (NOT realized held-out value)
        else:
            row[f] = exo_row[f]      # calendar / known-in-advance, by name from the held-out row
    return row


def build_recursive_backtest_predictions(model: lgb.LGBMRegressor, panel: pd.DataFrame,
                                         test: pd.DataFrame, cutoff: pd.Timestamp,
                                         features: list[str], sku_categories: pd.CategoricalDtype,
                                         horizon: int) -> pd.DataFrame:
    """Fixed-origin recursive multi-step backtest, per SKU.

    Never uses precomputed test-period target lag/rolling columns. For each SKU the running
    series is seeded with REAL units_observed on/before the cutoff, then each held-out date is
    predicted chronologically and folded back so day-2+ lags come from earlier PREDICTIONS,
    not earlier held-out actuals. Emits exactly the evaluator's eligible scoreable keys.
    """
    _assert_no_forbidden_columns(features)
    rows: list[dict] = []
    for sku, grp in test.sort_values(["sku", "date"]).groupby("sku"):
        hist = panel[(panel["sku"] == sku) & (panel["date"] <= cutoff)].sort_values("date")
        series = pd.Series(hist[TARGET].astype(float).to_numpy(),
                           index=pd.DatetimeIndex(hist["date"]))

        # origin-safe exogenous values (known at the cutoff), reused for every held-out day:
        # last known price carried forward; no promo / no discount unless a plan exists.
        origin_exog: dict = {}
        if "effective_unit_price" in features:
            price_hist = hist["effective_unit_price"].dropna() if "effective_unit_price" in hist else pd.Series(dtype=float)
            origin_exog["effective_unit_price"] = float(price_hist.iloc[-1]) if len(price_hist) else np.nan
        if "on_promo" in features:
            origin_exog["on_promo"] = 0
        if "discount_pct" in features:
            origin_exog["discount_pct"] = 0.0

        for _, exo_row in grp.iterrows():                 # chronological within the SKU
            date = exo_row["date"]
            feat = _recursive_feature_row(sku, date, series, exo_row, features, origin_exog)
            X = pd.DataFrame([feat])[features]
            X["sku"] = X["sku"].astype(sku_categories)
            y_pred = float(np.clip(model.predict(X)[0], 0, None))
            series.loc[date] = y_pred                     # feed prediction back for later held-out lags
            if bool(exo_row["forecast_training_eligible"]):
                rows.append({"sku": sku, "channel": exo_row["channel"], "date": date, "y_pred": y_pred})
    return pd.DataFrame(rows, columns=["sku", "channel", "date", "y_pred"])


# ── backtest: one horizon (fit unchanged; held-out prediction is recursive) ───────────
def run_backtest(panel: pd.DataFrame, horizon: int, features: list[str],
                  sku_categories: pd.CategoricalDtype) -> tuple[pd.DataFrame, dict, lgb.LGBMRegressor]:
    cutoff, train_all, test_all = ev.backtest_split(panel, horizon)
    train_eligible = train_all[train_all["forecast_training_eligible"].astype(bool)].sort_values("date")

    # small internal validation carve-out (last 14 eligible train days) for early
    # stopping only -- entirely inside the training period, never overlaps the test window.
    val_start = train_eligible["date"].max() - pd.Timedelta(days=14)
    fit_train = train_eligible[train_eligible["date"] < val_start]
    fit_val = train_eligible[train_eligible["date"] >= val_start]

    X_train = _prep_X(fit_train, features, sku_categories)
    y_train = fit_train[TARGET].clip(lower=0)
    X_val = _prep_X(fit_val, features, sku_categories)
    y_val = fit_val[TARGET].clip(lower=0)

    model = lgb.LGBMRegressor(**_lgbm_params())
    model.fit(
        X_train, y_train,
        eval_set=[(X_val, y_val)],
        eval_metric="mae",
        categorical_feature=["sku"],
        callbacks=[lgb.early_stopping(stopping_rounds=50, verbose=False)],
    )

    # leakage-free fixed-origin recursive forecast over the held-out window
    preds = build_recursive_backtest_predictions(
        model, panel, test_all, cutoff, features, sku_categories, horizon)

    result = ev.evaluate(preds, horizon=horizon, panel=panel)   # only [sku, channel, date, y_pred]

    # standardized backtest slice (NO y_true persisted) — shared contract columns
    backtest = preds.copy()
    backtest["model"] = MODEL_NAME
    backtest["horizon"] = int(horizon)
    backtest["origin"] = cutoff.date().isoformat()
    backtest["evaluation_type"] = "locked_holdout"
    return backtest, result, model


# ── future forecast: recursive, single model fit on all eligible history ──────────
def _future_row_features(sku: str, date: pd.Timestamp, series: pd.Series,
                          future_row: pd.Series, features: list[str]) -> dict:
    """One feature row for (sku, date) in the future window.

    Lag/rolling come from `series` (real history + predictions-so-far for this SKU),
    causally: only points strictly before `date` are ever read, matching the
    shift(1)-before-rolling construction in prepare_pilot_data.py.
    """
    before = series[series.index < date]
    window7 = before.iloc[-7:] if len(before) else before
    window28 = before.iloc[-28:] if len(before) else before

    row = {
        "sku": sku,
        "units_lag_1": before.iloc[-1] if len(before) >= 1 else np.nan,
        "units_lag_7": series.get(date - pd.Timedelta(days=7), np.nan),
        "units_lag_14": series.get(date - pd.Timedelta(days=14), np.nan),
        "units_roll_mean_7": window7.mean() if len(window7) else np.nan,
        "units_roll_mean_28": window28.mean() if len(window28) else np.nan,
        "units_roll_std_7": window7.std() if len(window7) >= 2 else np.nan,
    }
    for model_col, frame_col in FUTURE_COLUMN_MAP.items():
        val = future_row[frame_col]
        if model_col == "discount_pct" and pd.isna(val):
            val = 0.0  # no planned promo -> no planned discount, matches historical convention
        row[model_col] = val
    return {k: row[k] for k in features}


def run_future_forecast(panel: pd.DataFrame, future: pd.DataFrame, features: list[str],
                         sku_categories: pd.CategoricalDtype, as_of_date: str) -> pd.DataFrame:
    train_eligible = panel[panel["forecast_training_eligible"].astype(bool)].sort_values("date")
    X_train = _prep_X(train_eligible, features, sku_categories)
    y_train = train_eligible[TARGET].clip(lower=0)

    model = lgb.LGBMRegressor(**_lgbm_params(n_estimators=300))  # no val split -> fixed tree count
    model.fit(X_train, y_train, categorical_feature=["sku"])

    rows: list[dict] = []
    for sku, sku_future in future.sort_values(["sku", "date"]).groupby("sku"):
        sku_hist = panel[panel["sku"] == sku].sort_values("date")
        series = pd.Series(sku_hist[TARGET].to_numpy(), index=sku_hist["date"])

        for _, future_row in sku_future.iterrows():
            date = future_row["date"]
            feat = _future_row_features(sku, date, series, future_row, features)
            X = pd.DataFrame([feat])
            X["sku"] = X["sku"].astype(sku_categories)
            y_pred = float(np.clip(model.predict(X[features])[0], 0, None))

            series.loc[date] = y_pred  # feed prediction back in for the next day's lags
            rows.append({
                "sku": sku,
                "product_id": future_row.get("product_id"),
                "channel": future_row["channel"],
                "date": date,
                "forecast_horizon_day": future_row["forecast_horizon_day"],
                "y_pred": y_pred,
                "as_of_date": as_of_date,
                "model_version": MODEL_VERSION,
            })

    return pd.DataFrame(rows)


def print_feature_importance(model: lgb.LGBMRegressor, features: list[str], horizon: int) -> None:
    imp = pd.Series(model.feature_importances_, index=features).sort_values(ascending=False)
    print(f"\ntop features @ horizon={horizon}:\n{imp.head(8).to_string()}")


def run(model_panel=None, forecast_frame=None, manifest=None,
        output_dir=OUT, horizons=HORIZONS) -> dict:
    """Run the pooled LightGBM suite against a prepared run and write shared-contract outputs.

    Paths default to data/processed/ + outputs/ (legacy). The backtest is a fixed-origin
    RECURSIVE multi-step forecast (leakage-free); the future forecast is recursive too.
    """
    warnings.filterwarnings("ignore", category=UserWarning)
    out_dir = Path(output_dir)
    horizons = tuple(int(h) for h in horizons)
    mp_path = Path(model_panel) if model_panel else DEFAULT_MODEL_PANEL
    ff_path = Path(forecast_frame) if forecast_frame else DEFAULT_FORECAST_FRAME
    man_path = Path(manifest) if manifest else MANIFEST_PATH

    panel, future, man = load_contract(mp_path, ff_path, man_path)
    as_of_date = man["as_of_date"]
    features = feature_list(man)
    sku_categories = pd.CategoricalDtype(categories=sorted(panel["sku"].unique()))
    print(f"Loaded model_panel: {panel.shape}, forecast_frame: {future.shape}")
    print(f"Features ({len(features)}): {features}")

    backtest_slices, score_rows = [], []
    for horizon in horizons:
        bt, result, _model = run_backtest(panel, horizon, features, sku_categories)
        backtest_slices.append(bt)
        score_rows.append(mc.scorecard_row(MODEL_NAME, horizon, result, "locked_holdout"))
        o = result["overall"]
        print(f"[lightgbm h={horizon} cutoff {result['cutoff']}] recursive backtest — "
              f"WAPE={o['wape']:.4f} MASE={o['mase']:.4f} MAE={o['mae']:.3f} "
              f"RMSE={o['rmse']:.3f} bias={o['bias']:.3f} n={result['n_test_rows']}")

    backtest = pd.concat(backtest_slices, ignore_index=True)
    backtest = mc.validate_backtest_predictions(backtest, MODEL_NAME, allowed_horizons=tuple(horizons))
    scorecard = pd.DataFrame(score_rows, columns=mc.SCORECARD_COLUMNS)

    future_preds = run_future_forecast(panel, future, features, sku_categories, as_of_date)
    future_preds["model"] = MODEL_NAME
    if "sku_name" in future.columns:                       # preserve product name when available
        name_map = future.drop_duplicates("sku").set_index("sku")["sku_name"]
        future_preds["sku_name"] = future_preds["sku"].map(name_map)
    future_preds = mc.validate_future_predictions(future_preds, future, man, MODEL_NAME)

    mc.write_dataframe_atomic(backtest, out_dir / "lightgbm_backtest_predictions.parquet", "parquet")
    mc.write_dataframe_atomic(scorecard, out_dir / "lightgbm_scorecard.csv", "csv")
    mc.write_dataframe_atomic(future_preds, out_dir / "future_forecast_lightgbm.parquet", "parquet")

    summary = {
        "model": MODEL_NAME,
        "model_version": MODEL_VERSION,
        "as_of_date": as_of_date,
        "horizons": list(horizons),
        "dataset_fingerprint": mc.dataset_fingerprint(mp_path, ff_path, man_path),
        "n_skus": int(panel["sku"].nunique()),
        "channel": EXPECTED_CHANNEL,
        "features": features,
        "hyperparameters": _lgbm_params(),
        "output_files": ["lightgbm_backtest_predictions.parquet", "lightgbm_scorecard.csv",
                         "future_forecast_lightgbm.parquet", "lightgbm_run_summary.json"],
        "backtest_method": "fixed-origin recursive multi-step forecast using predicted demand for future lags",
        "future_method": "recursive multi-step forecast using predicted demand for future lags",
        "backtest_exogenous_policy": BACKTEST_EXOG_POLICY,
    }
    mc.write_json_atomic(summary, out_dir / "lightgbm_run_summary.json")

    print("\n============= lightgbm suite complete =============")
    print(f"output-dir: {out_dir}  horizons: {list(horizons)}  n_skus: {summary['n_skus']}")
    print(f"future rows: {len(future_preds)}  backtest rows: {len(backtest)}")
    print(f"dataset fingerprint: {summary['dataset_fingerprint'][:16]}…")
    print("NOTE: backtest is fixed-origin recursive & leakage-free; future forecast is unscored.")
    return summary


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Pooled LightGBM demand forecaster (run-aware).")
    ap.add_argument("--model-panel", default=None,
                    help="model_panel.parquet (default: data/processed/model_panel.parquet)")
    ap.add_argument("--forecast-frame", default=None,
                    help="forecast_frame.parquet (default: data/processed/forecast_frame.parquet)")
    ap.add_argument("--manifest", default=None,
                    help="pilot_manifest.json (default: data/processed/pilot_manifest.json)")
    ap.add_argument("--output-dir", default=str(OUT))
    ap.add_argument("--horizons", nargs="+", type=int, choices=[7, 14], default=[7, 14])
    return ap.parse_args(argv)


def main(argv: list[str] | None = None) -> int:
    args = parse_args(argv)
    run(model_panel=args.model_panel, forecast_frame=args.forecast_frame,
        manifest=args.manifest, output_dir=args.output_dir, horizons=tuple(args.horizons))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
