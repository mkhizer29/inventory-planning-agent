...
# ── SciPy/statsmodels environment-compat shim (NOT an algorithm change) ──────────────
# statsmodels 0.14.x forwards a `disp` kwarg to SciPy's fmin_l_bfgs_b, which newer SciPy
# versions no longer accept (raises TypeError, so every ETS fit would crash and the model
# would silently degrade to seasonal-naive). We strip only that unsupported verbosity flag
# at the optimizer boundary: the SAME L-BFGS-B optimizer runs and the fitted parameters /
# forecasts are identical. Where SciPy still accepts `disp`, this shim is a harmless no-op.
import scipy.optimize as _spopt  # noqa: E402
if not getattr(_spopt.fmin_l_bfgs_b, "_ipa_disp_shim", False):
    _ipa_orig_lbfgsb = _spopt.fmin_l_bfgs_b

    def _ipa_lbfgsb(*args, **kwargs):
        kwargs.pop("disp", None)
        return _ipa_orig_lbfgsb(*args, **kwargs)

    _ipa_lbfgsb._ipa_disp_shim = True
    _spopt.fmin_l_bfgs_b = _ipa_lbfgsb
...
# ── contract load + audit ───────────────────────────────────────────────────────────
def load_contract(model_panel=MODEL_PANEL, forecast_frame=FORECAST_FRAME,
                  manifest=MANIFEST) -> tuple[pd.DataFrame, pd.DataFrame, dict]:
    """Load the three shared inputs from the given paths (defaults = data/processed)."""
    model_panel, forecast_frame, manifest = Path(model_panel), Path(forecast_frame), Path(manifest)
...
def run_locked_backtests(series: dict[str, pd.DataFrame], selection: dict[str, dict],
                         panel: pd.DataFrame, max_date: pd.Timestamp,
...
def save_outputs(sel_metrics: pd.DataFrame, locked: dict, prod: pd.DataFrame,
                 selection: dict, audit: dict, man: dict, ff: pd.DataFrame,
                 output_dir=OUT, mp_path=MODEL_PANEL, ff_path=FORECAST_FRAME,
                 man_path=MANIFEST, horizons: tuple[int, ...] = HORIZONS) -> dict:
    out = Path(output_dir)
    out.mkdir(parents=True, exist_ok=True)
    mp_path, ff_path, man_path = Path(mp_path), Path(ff_path), Path(man_path)

    # 1) backtest metrics (diagnostic): selection folds + locked selected + seasonal-naive benchmark
    sel_m = sel_metrics.assign(evaluation_type="selection_fold")
    locked_rows = []
    for h in horizons:
        sc = locked["scorecards"][h]; o = sc["overall"]
        locked_rows.append({"evaluation_type": "locked_selected", "horizon": h, "origin": sc["cutoff"],
                            ...
                            "mase": o["mase"]})
        sn = locked["seasonal_naive"][h]; o2 = sn["overall"]
        locked_rows.append({"evaluation_type": "seasonal_naive_locked", "horizon": h, "origin": sn["cutoff"],
                            "model_id": "seasonal_naive_7", "fit_status": "benchmark", "n_val": sn["n_test_rows"],
                            "mae": o2["mae"], "rmse": o2["rmse"], "wape": o2["wape"], "bias": o2["bias"],
                            "mase": o2["mase"]})
    ...
    mc.write_dataframe_atomic(metrics, out / "holtwinters_backtest_metrics.csv", "csv")

    # 2) backtest predictions — shared contract (model="holtwinters"), validated
    bt_pred = pd.DataFrame(locked["pred_rows"])
    bt_pred["model"] = "holtwinters"
    bt_pred = mc.validate_backtest_predictions(bt_pred, "holtwinters", tuple(horizons))
    mc.write_dataframe_atomic(bt_pred, out / "holtwinters_backtest_predictions.parquet", "parquet")

    # 3) scorecard — shared contract; the locked ETS holdouts scored by evaluation.evaluate
    scorecard = pd.DataFrame(
        [mc.scorecard_row("holtwinters", h, locked["scorecards"][h], "locked_holdout") for h in horizons],
        columns=mc.SCORECARD_COLUMNS)
    mc.write_dataframe_atomic(scorecard, out / "holtwinters_scorecard.csv", "csv")

    # 4) future forecast — shared contract (model="holtwinters"), validated vs forecast_frame
    future = prod.copy()
    future["model"] = "holtwinters"
    future = mc.validate_future_predictions(future, ff, man, "holtwinters")
    mc.write_dataframe_atomic(future, out / "future_forecast_holtwinters.parquet", "parquet")

    # 5) model selection JSON (rich detail preserved; fingerprints from the passed input paths)
    sel_counts = {}
    ...
    doc = {
        ...
        "source_fingerprints_sha256": {"model_panel.parquet": file_sha256(mp_path),
                                       "forecast_frame.parquet": file_sha256(ff_path),
                                       "pilot_manifest.json": file_sha256(man_path)},
    ...
    mc.write_json_atomic(doc, out / "holtwinters_model_selection.json")

    # 6) run summary — shared dataset fingerprint + run metadata
    summary = {
        "model": "holtwinters",
        "model_version": MODEL_VERSION,
        "as_of_date": man["as_of_date"],
        ...
        "output_files": ["holtwinters_backtest_predictions.parquet", "holtwinters_scorecard.csv",
                         "future_forecast_holtwinters.parquet", "holtwinters_model_selection.json",
                         "holtwinters_run_summary.json"],
    }
    mc.write_json_atomic(summary, out / "holtwinters_run_summary.json")
    return {"metrics": metrics, "bt_pred": bt_pred, "prod": future, "doc": doc,
            "scorecard": scorecard, "summary": summary}


# ── pipeline ─────────────────────────────────────────────────────────────────────────
def run_pipeline(model_panel=MODEL_PANEL, forecast_frame=FORECAST_FRAME, manifest=MANIFEST,
                 output_dir=OUT, horizons=HORIZONS) -> dict:
    horizons = tuple(int(h) for h in horizons)
    mp, ff, man = load_contract(model_panel, forecast_frame, manifest)
    ...
    locked = run_locked_backtests(series, selection, mp, max_date, horizons)
    for h in horizons:
    ...
    saved = save_outputs(sel_metrics, locked, prod, selection, audit, man, ff,
                         output_dir, model_panel, forecast_frame, manifest, horizons)
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="Univariate ETS demand forecaster (Stage A, run-aware).")
    ap.add_argument("--model-panel", default=str(MODEL_PANEL))
    ap.add_argument("--forecast-frame", default=str(FORECAST_FRAME))
    ap.add_argument("--manifest", default=str(MANIFEST))
    ap.add_argument("--output-dir", default=str(OUT))
    ap.add_argument("--horizons", nargs="+", type=int, choices=[7, 14], default=list(HORIZONS))
    ap.add_argument("--selfcheck", action="store_true", help="recompute in-process and assert identical")
    args = ap.parse_args(argv)
    run_kwargs = dict(model_panel=args.model_panel, forecast_frame=args.forecast_frame,
                      manifest=args.manifest, output_dir=args.output_dir, horizons=tuple(args.horizons))
    first = run_pipeline(**run_kwargs)
    if args.selfcheck:
        self_check(first, **run_kwargs)
    print(f"\nDONE. Outputs in {args.output_dir}. Demand is REAL units_observed; model is univariate "
          "per SKU; no synthetic inventory used; accuracy is backtest-ESTIMATED, not guaranteed.")
    return 0
